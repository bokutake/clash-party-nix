...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    test -f "$out/bin/clash-party"
    test -f "$out/lib/clash-party/mihomo-party"
...
PERL
    ...
    for sidecar in mihomo mihomo-alpha mihomo-smart; do
      test -f "$out/lib/clash-party/resources/nix-sidecar-store/$sidecar.bin.real"
    done
  ...
  meta = with lib; {
    description = "Wrapped Clash Party package for NixOS and Home Manager";
